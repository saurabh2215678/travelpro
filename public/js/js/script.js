...
$(window).on('resize', function(){
    if(window.innerWidth < 1200){
        if($('header').find('.sidemenu-box').length == 0){
            $('.theme-nav').css('display', 'flex');
            $('.social-icons').css('display', 'flex');
            $('.social-icons, .theme-nav').wrapAll( "<div class='sidemenu-box'></div>" );
            $(`
...
equalheight('.exhibition-list', '.para-md');
    ...
    //     var maxHeight = 0;
    //     $(this).find('.para-md').height('auto');
    //     $(this).find('.para-md').each(function (index) {
    //         if ($(this).height() > maxHeight)
    //             maxHeight = $(this).height();
    //     });
    //     $(this).find('.para-md').height(maxHeight);
    // });
