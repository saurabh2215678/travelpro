$(window).on('resize', function(){
    if(window.innerWidth < 1200){
        if($('header').find('.sidemenu-box').length == 0){
            $('.theme_nav_search .theme-nav').css('display', 'flex');
            $('.social-icons').css('display', 'flex');
            $('.social-icons, .theme_nav_search .theme-nav').wrapAll( "<div class='sidemenu-box'></div>" );
            $(`
        ...
        if($('header').find('.sidemenu-box').length > 0){
            $('.social-icons, .theme_nav_search .theme-nav').unwrap();
            $('.more-menu .header-backdroap, .phone-menu, .dd-click').remove();
        }
        $('.sub-menu').show();
    }
