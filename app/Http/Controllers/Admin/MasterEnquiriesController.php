...
class MasterEnquiriesController extends Controller
{
    // private $limit;
    ...
    public function index(Request $request){
        ...
        $records = EnquiriesMaster::with('GroupData')->withCount('child')->where('parent_id',$parent_id)->where('type',$type)->orderBy('sort_order','asc');
        if($parent_id){
        ...
        $data['type'] = $type;
        return view('admin.master_enquiries.index', $data);
    }
    ...
    public function add(Request $request){
        ...
        $title = '';
        if($type == 'lead-status'){ $title = 'Add Lead Status'; } elseif($type == 'lead-source'){ $title = 'Add Lead Source'; } elseif($type == 'rating'){ $title = 'Add Rating'; } elseif($type == 'order-status'){ $title = 'Add Order Status'; }

        if(is_numeric($id) && $id > 0){
            $record = EnquiriesMaster::find($id);
            if($type == 'lead-status'){ $title = 'Edit Lead Status'; } elseif($type == 'lead-source'){ $title = 'Edit Lead Source'; } elseif($type == 'rating'){ $title = 'Edit Rating'; } elseif($type == 'order-status'){ $title = 'Edit Order Status'; }
        }
        if($request->method() == 'POST' || $request->method() == 'post'){
            ...
            if ($isSaved) {
                ...
                $back_url = url(CustomHelper::getAdminRouteName().'/master_enquiries?type='.$type);
        ...
        $data['id'] = $id;
        $data['type'] = $type;
        return view('admin.master_enquiries.form', $data);
    }
