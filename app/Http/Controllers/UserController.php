...
public function cab(Request $request) {
	...
	if(!empty(Auth::check())) {
		if(config('custom.CAB_VERSION') == 2) {
			$query = CabsSightseeing::with('originCity','destinationCity')->where('status',1);
			if($request->name) {
				$name = $request->name??'';
			...
			$cabsSightseeings = $query->orderBy('sort_order', 'asc')->paginate($limit);
			$results_arr = [];
			foreach ($cabsSightseeings as $key => $cabsSightseeing) {
				$results_arr[] = CabsSightseeing::parseCabsSightseeing($cabsSightseeing, ['size'=>'detail']);
			}
			$results = [];
			$results['data'] = $results_arr;
			...
			return view('user.cab_version2', $data);
...
public function cab_price(Request $request) {
	...
	if(!empty($id)) {
		...
		if(config('custom.CAB_VERSION') == 2) {
			$sightseeingData = CabsSightseeing::find($id);
			if($sightseeingData) {

				$cabsCityPrices = $sightseeingData->originCity->cabsCityPrices??[];
				$car_arr = [];
				if($cabsCityPrices) {
					foreach($cabsCityPrices as $cabsCityPrice) {
						$car_arr[]  = CabsCityPrice::parseCabsCityPrice($cabsCityPrice);
					}
				}
				...
				$data["heading"] = 'Agent Price ('.$sightseeingData->name.')';

				// prd($data['discount_module_groups']);
				$data["sightseeingData"] = $sightseeingData;
				...
				return view('user.cab_price_view_version2', $data);
			}		
		} else {
...
public function orderDetail(Request $request) {
	...
	if (is_numeric($id) && $id > 0 && !empty($userId)) {
		...
		if($order) {
			...
			$show_cancel_confirm = 0;
			if($request->show_cancel_confirm) {
				$show_cancel_confirm = $request->show_cancel_confirm??0;
			}
			$data['show_cancel_confirm'] = $show_cancel_confirm;
...
public function updatePNRDetails(Request $request) {
	$response = [];
	$order_id = isset($request->order_id) ? $request->order_id : 0;
	$order = "";
	$user_id = auth()->user() ? auth()->user()->id :0;
	if (is_numeric($order_id) && $order_id > 0 && !empty($user_id)) {
		$order = Order::find($order_id);
		if($order && $order->supplier_id == $user_id){
			$user_name = auth()->user() ? auth()->user()->name:'system';
			...
			if($isSaved) {
				$response['success'] = true;
				$response['msg'] = ' PNR Details has been updated successfully';
			} else {
				$response['success'] = false;
				$response['msg'] = ' PNR Details has been not been updated, please try again or contact the adminstrator!';
			}
		} else {
			$response['success'] = false;
			$response['msg'] = 'Invalid request';
...
public function cancelOfflineFlight(Request $request) {
	...
	if (is_numeric($order_id) && $order_id > 0 && !empty($user_id)) {
		$order = Order::find($order_id);
		if($order && $order->supplier_id == $user_id && $order->status != 'Cancelled'){
			...
			if($resp) {
				$response['success'] = true;
				$response['msg'] = ' Flight has been cancelled successfully';
			} else {
				$response['success'] = false;
				$response['msg'] = ' Flight has been not been cancelled, please try again or contact the adminstrator!';
			}
		} else {
			$response['success'] = false;
			$response['msg'] = 'Invalid request';
...
public function record_package_favourite(Request $request)
{
	...
	if(Auth::check())
	{
		...
		public function flight_voucher_view(Request $request) {
			...
			if(!empty($order_id) && !empty($user_id)) {
				...
				$query->where(function($q1) use($user_id) {
					$q1->where('user_id',$user_id);
					$q1->orWhere('supplier_id',$user_id);
				});
				$order = $query->first();
            	// prd($order->toArray());
	...
	public function flight_route(Request $request) {
		$data = [];
		$user_id = Auth::user()->id??0;
		$is_agent = Auth::user()->is_agent??0;
		$is_allowed_offline_flight_inventory = Auth::user()->agentInfo->is_allowed_offline_flight_inventory??0;
		if($is_agent==0 || $is_allowed_offline_flight_inventory==0) {
			abort(404);
		}
        $limit = 50;
        $status = (isset($request->status))?$request->status:'';
        $query = AirlineRoute::orderBy('featured','desc')->orderBy('sort_order','asc')->orderBy('id','desc');
        $query->where('user_id', $user_id);
        if($request->name) {
            $name = $request->name;
		...
		$data['page_heading'] = 'Offline Flight Route';
		$data['meta_title'] = 'Offline Flight Route - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
		$data['meta_description'] = 'Offline Flight Route - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');

		return view('user.flight_route',$data);
	}

	public function flight_route_add(Request $request) {
		$data = [];
		$user_id = Auth::user()->id??0;
		$user_name = auth()->user()->name;
		$is_agent = Auth::user()->is_agent??0;
        ...
        if(is_numeric($id) && $id > 0){
            $record = AirlineRoute::where('id',$id)->where('user_id',$user_id)->first();
            if($record && $record->id) {
		...
		$data['page_heading'] = $title;
		$data['meta_title'] = 'Manage Flight Routes - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
		$data['meta_description'] = 'Manage Flight Routes - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
		$data['record'] = $record;
        $data['id'] = $id;
		return view('user.flight_route_form',$data);
	}

	public function flight_route_view(Request $request) {
		$data = [];
		$user_id = Auth::user()->id??0;
		$is_agent = Auth::user()->is_agent??0;
		if($is_agent==0) {
		...
		if(is_numeric($id) && $id > 0) {
			...
			$data = [];
			$data['page_heading'] = $title;
			$data['meta_title'] = 'Flight Inventory - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
			$data['meta_description'] = 'Flight Inventory - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
			$data['record'] = $record;
			$data['id'] = $id;
			return view('user.flight_route_view',$data);
		} else {
			abort(404);
		}
	}

	public function flight_inventory(Request $request) {
		$data = [];
		$user_id = Auth::user()->id??0;
		$is_agent = Auth::user()->is_agent??0;
		$is_allowed_offline_flight_inventory = Auth::user()->agentInfo->is_allowed_offline_flight_inventory??0;
		...
		$query = AirlineRouteInventory::where('user_id',$user_id)->orderBy('start_date','asc')->orderBy('id','desc');
		if($request->name) {
			$name = $request->name;
			$query->where(function($q1) use($name) {
				$q1->orWhereHas('routeData',function($q2) use($name) {
					$q2->where('name','like','%'.$name.'%');
					$q2->orWhere('source',$name);
		...
		if($request->route_id) {
			$query->where('airline_route_id', $request->route_id);
		}
		if($status !== '') {
			$query->where('status', $status);
		}
		...
		$data['page_heading'] = 'Offline Flight Inventory';
		$data['meta_title'] = 'Offline Flight Inventory - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
		$data['meta_description'] = 'Offline Flight Inventory - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');

		return view('user.flight_inventory',$data);
	}

	public function flight_inventory_add(Request $request) {
		$data = [];
		$user_id = Auth::user()->id??0;
		$is_agent = Auth::user()->is_agent??0;
		if($is_agent==0) {
		...
		if (is_numeric($id) && $id > 0) {
			...
			if($record && $record->id && $record->user_id == $user_id) {
		...
		if($request->method() == 'POST') {
			...
			$validator->after(function($validator) use ($request){
                ...
                if(1 == 2) { // Allow the selling price to be less than actual
			...
			if ($validator->fails()) {
				...
				$req_data = [
					...
					'agent_adult_price' => $request->agent_adult_price??0,
					'agent_child_price' => $request->agent_child_price??0,
					'agent_infant_price' => $request->agent_infant_price??0,
					'adult_price' => $request->adult_price??0,
					'child_price' => $request->child_price??0,
					'infant_price' => $request->infant_price??0,
                    'admin_adult_price' => $request->adult_price??0,
                    'admin_child_price' => $request->child_price??0,
                    'admin_infant_price' => $request->infant_price??0,
				...
				if($isSaved) {
					if($request->back_url) {
						...
						$back_url = route('user.flight_inventory');
					}					
					return redirect($back_url)->with('alert-success', $msg);
				} else {
					return back()->with('alert-danger', 'Something went wrong, please try again...');
		...
		$data['meta_title'] = 'Flight Inventory - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
		$data['meta_description'] = 'Flight Inventory - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
		return view('user.flight_inventory_form',$data);
	}

	public function flight_inventory_view(Request $request) {
		$data = [];
		$user_id = Auth::user()->id??0;
		$is_agent = Auth::user()->is_agent??0;
		if($is_agent==0) {
		...
		if (is_numeric($id) && $id > 0) {
			...
			if($record && $record->id && $record->user_id == $user_id) {
				...
				$data['meta_title'] = 'Flight Inventory - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
				$data['meta_description'] = 'Flight Inventory - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
				return view('user.flight_inventory_view',$data);
			} else {
				abort(404);
			}
		} else {
    ...
    public function myFlightBooking(Request $request) {
		$data = [];
		$limit = $this->limit;
		$userId = "";
		if(!empty(Auth::check())) {
			...
			$query->where('order_type',3)->where('order_type','!=',7);
			/*$query->where(function($q1) use($userId) {
				$q1->orWhere('user_id',$userId);
			});*/
			$query->where('supplier_id',$userId);
			$order_type = $request->order_type??'pending';
			switch ($order_type) {
				case 'bookings':
			...
			$data['uesrOrders'] = $orders;
			$data['page_heading'] = 'Offline Flight Booking';
			$data['meta_title'] = 'My Booking - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
			$data['meta_description'] = 'My Booking - '.CustomHelper::WebsiteSettings('SYSTEM_TITLE');
			$data['from_date'] = $from_date;
			$data['to_date'] = $to_date;
			...
			unset($search_data['order_type']);
			$data['search_data'] = $search_data;
			return view('user.my_flight_booking', $data);
		} else {
			return abort(404);
		}
	}
