<template>
    ...
    <HomePackageSlider
        ...
        :sectionData="{data: popular_packages, links: tourPackagesUrl}"
        :title="`Places to Visit in ${destination?.name}`"
        :hiderating="true"
    />

    <!-- <DestinationPackageSlider v-if="popular_packages && popular_packages.length > 0" :destination="destination" :packages="popular_packages" :viewAllUrl="tourPackagesUrl" /> -->
    ...
    <NoDataWrapper class="no_data py-10" v-else>
        <div class="container">
            <div class="no_data_inner">
                ...
                <div class="text">Kindly contact our sale team <a :href="`mailto:${store.websiteSettings?.SALES_EMAIL}`">({{store.websiteSettings?.SALES_EMAIL}}) </a> for more information.</div>
            </div>
        </div>
    </NoDataWrapper>
    ...
    <template v-if="popular_activities && popular_activities.length > 0">
        ...
        <HomePackageSlider
            :sectionData="{data: popular_activities, links: tourActivitiesUrl}"
    ...
    <DestinationDetalisGallery v-if="destination?.images && destination?.images?.length > 0" :images="destination.images" title="Gallery" /> 
