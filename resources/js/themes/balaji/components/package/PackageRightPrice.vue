<template>
    ...
    <div class="city-list-block">
            ...
            <div id="element" class="btn btn-default show-modal mt-5" @click="showItenaryPopup" v-if="store?.myEvents?.length">Download Itinerary</div>
        </div>
        <form id="bookNowForm">
        ...
        <div class="flex py-0 pt-2 items-center">
            <div class="bg_share">
...
import interactionPlugin from '@fullcalendar/interaction'
    ...
    export default {
                    ...
                    $('.right_side').find(".element_choice").each(function( index ) {
                        ...
                        if(is_calculate==1) {
                            ...
                            if(parseInt(selected_single_price_value) > 0){
                                $('.right_side').find('.price_select'+elementId).html(showPrice(selected_single_price_value,true));
                            } else {
                                $('.right_side').find('.price_select'+elementId).html('');
                            }
                            if(original_price_val != selected_price_value){
                                $('.right_side').find('.price_select'+elementId).html('<s>'+this.showPrice(original_price_val,true)+'</s> '+this.showPrice(selected_price_value,true));
                            }
                        }
                    });
                    currentObj.final_pay_price = total_final_val;

                    //$('.right_side').find('#final_pay_price').html(showPrice(total_final_val,true));
        ...
        layout: Layout,
        props: ["package","defaultPriceId","faq_row","destinations","itenaries", "calenderPop", "showCalenderPopup", "hideCalenderPopup","sharing_links"],
    };
    </script>