            ...
            <div class="detail_tab_section">
                <div class="container">
                    <ul class="scrollspy_btns" ref="navRef">
                        <li @click="handleActive('dest_about')"><a href="#dest_about" class="active">About {{destination?.name}}</a></li>
                        <template v-if="destination.destinationTypes" v-for="destinationType in destination.destinationTypes">
                            <li @click="handleActive(destinationType.slug)"><a :href="`#${destinationType.slug}`">{{destinationType?.name}}</a></li>
                        </template>
                        <li v-if="destination.images" @click="handleActive('dest_gallery')"><a href="#photo_gallery">Photo Gallery</a></li>
                    </ul>
                </div>
            </div>
            <div class="inner_content_block">
                <div class="container">
                    <div id="dest_about" class="tabcontent" scrollspy-section>
                        <div class="font30 color_theme fw700 mb-8">About</div>
                        <div class="destination_about_content flex gap-x-3">
                            <div class="dest_desc w-full md:pr-5 text-gray-500">
                    ...
                    <template v-if="destination.destinationTypes" v-for="destinationType in destination.destinationTypes">
                        <div :id="destinationType.slug" class="tabcontent" scrollspy-section>
                            <div class="font30 color_theme fw700 mb-8">{{destinationType?.name}}</div>
