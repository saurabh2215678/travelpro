        ...
        <table class="table table-bordered">
            <tbody>
                <tr>
                    ...
                    <td>
                        <h3 class="text-blue-800 text-sm font-semibold"> <a class="flex" href="javascript:void(0)" @click="handlePopup">
                         <!--<img title="Shared cab available" class="sightseen_share_icon mr-2" src="../../assets/images/cab-sharing.png" style="height: 1.5rem;" v-if="row.sharing == 1" />-->
                    ...
                    <td align="center" class="text-sm"><i class="fas fa-tachometer-alt text-teal-500" v-if=row.distance></i><br> <span v-if=row.distance>{{row.distance}} km</span></td>
                    <td><h4 class="text-sm p-1">Starting From :</h4>
                        <div class="ss_price text-blue-800 text-lg font-semibold"><i class="fa-solid fa-indian-rupee-sign"></i>{{row.price}}</div>
                        <div class="shapeType2 text-lg bg-amber-600 inline-block text-white p-0.5 rounded-sm" v-if="row.sharing == 1" >
                            Shared Cab
                        </div>
                    </td>
                    <td><a :href="row.book_url"><button class="bookbtn rounded-full px-5 py-2"> Book Now</button></a> </td>
                </tr>
            </tbody>
        </table>
    </div>
...
<script>
import {showPopup, hidePopup} from '../../utils/commonFuntions.js';
