<template>
    ...
    <SearchFormWithBanner title="Search For Place"  />
    ...
    <div class="text_center"> 
        <div class="theme_title mb-6">
            ...
            <div class="breadcrumb_full">
                <div class="container">
                    <div class="theme-title flex justify-between flex-wrap">
                        <h1 class="title text-2xl mb-5">{{destination?.name}}</h1>
                        <ul class="breadcrumb">
                            <li><Link :href="store.websiteSettings?.FRONTEND_URL">{{showLangTitle('Home')}}</Link></li>
                            <li><Link :href="store.websiteSettings?.DESTINATION_URL">{{showLangTitle('Destinations')}}</Link></li>
    ...
    /> -->
    <template v-if="newsBlogs && newsBlogs.length > 0">
        <BlogSlider 
        :data="newsBlogs"
        subtitle="Featured"
        :title="showLangTitle('Blogs&News')"
        />
    </template>

    <DestinationTestimonialSlider v-if="testimonials && testimonials.length > 0" :destination="destination" :testimonials="testimonials" :testimonialsUrl="testimonialsUrl" />
    <!-- <div class="pt-16"></div> -->
    <DestinationFaqSlider v-if="faqs && faqs.length > 0" :destination="destination" :faqs="faqs" />
...
import DestinationPackageSlider from '../components/destination/DestinationPackageSlider.vue';
import DestinationActivitySlider from '../components/destination/DestinationActivitySlider.vue';
import DestinationTab from '../components/destination/DestinationTab.vue';
import DestinationFaqSlider from '../components/destination/DestinationFaqSlider.vue';
import DestinationTestimonialSlider from '../components/destination/DestinationTestimonialSlider.vue';
import SearchFormWithBanner from '../components/SearchFormWithBanner.vue';
import PageTopInfo from "../components/destination/PageTopInfo.vue";
import {NoDataWrapper} from '../styles/noDataWrapper';
import DestinationDetalisGallery from "../components/destination/DestinationDetalisGallery.vue";
...
export default {
    ...
    components: {
        ...
        DestinationActivitySlider,
        DestinationTab,
        DestinationFaqSlider,
