      ...
      <div class="packaging_view_box_top p-3">
        ...
        <div class="packaging_info">
          <div class="package_info_wrap">
            <div class="package_top">
              <div class="title para_lg2">
                <span class="package_tour_type_text">{{packageData.tour_type? packageData.tour_type :''}} {{showLangTitle('Package')}}</span>
              ...
              <div class="price_inner mb-2" v-if="packageData.packagePrices && packageData.packagePrices.length > 0">
                <p class="price_package text-lg text-black">{{showLangTitle('StartingFrom')}} : </p>
              ...
              <div class="price_inner mb-2" v-else>
                <div class="req_price" v-if="packageData.packagePrices && packageData.packagePrices.length <= 0">Prezzo su richiesta</div>
              </div>
          ...
          <div class="cities pb-2" v-if="packageData?.days_nights_city?.length > 0">
            <strong>{{showLangTitle('Location')}} : </strong>
        ...
        <div class="packaging_view_footer_inner">
          ...
          <div class="right_side">
            <div class="flex flex-wrap my-3">
                <Link :href="this.bookingUrl" class="btn mr-3 text-sm bg_theme mb-2" v-if="this.publish_price && this.search_price && parseFloat(this.publish_price) > parseFloat(this.search_price) && packageData.packagePrices.length > 0">{{showLangTitle('BookNow')}}</Link>
                <Link :href="`${packageData.enquiry_url}&type=${this.typename}`" class="btn theme_clr enquir_now text-sm bg_secondary_theme mb-2">{{showLangTitle('EnquireNow')}}</Link>
              </div>
          </div>
        </div>
      </div>
  ...
  import {showPrice, showLangTitle} from '../../utils/commonFuntions.js';
