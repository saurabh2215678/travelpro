<template>
    <div class="right_price_box">
        <form id="bookNowForm">
            <div class="par_cost pb-2 text-sm">{{showLangTitle('From')}} <span>{{showPrice(this.startingPrice)}}</span> {{showLangTitle('PerPerson')}}</div>
            <div class="scac flex items-center">
                <ul class="form_list w-full float-left">
                    <li class="w-2/4">
                        <div class="package_type">
                            <label class="text-sm">{{showLangTitle('PackageType')}}</label>
            ...
            <div class="single_package_day">
                <ul class="form_list w-full float-left mt-2">
                    <li class="w-2/4" v-if="this.package_tour_type != 'open' && store?.myEvents?.length > 0">
                        <div class="form_group icon_calender inline-block">
                            <label class="text-sm">{{showLangTitle('TripDate')}}</label>
                    ...
                    <li class="w-2/4">
                        <label class="text-sm font-bold">{{showLangTitle('NumberofPersons')}}</label>
                        <fieldset class="scheduler-border booking_fields">
                            <legend class="select_trav font-normal" @click="toggleTraveller">{{showLangTitle('SelectTravellers')}}</legend>
                            ...
                            <div class="single_package_white" :class="selectTraveller ? 'active' : ''">
                                <div class="single_btn">
                                    <ul class="flex flex-wrap">
                                        ...
                                        <li class="li_last"><label class="text-xs"> </label><a class="apbtn" href="#" id="applyTravellers" @click="this.handleApplyTravellers" >{{showLangTitle('Apply')}}</a></li>
                                    </ul>
                                </div>
                            </div>
                        </fieldset>
            ...
            <div class="slot_time" v-if="this.package_tour_type != 'open' && this.isOnlineBooking('activity_listing') && store?.myEvents?.length > 0">
                <a class="apply_slot" href="#" id="applyTripTravellers" @click="checkSlotAvailability">{{showLangTitle('CheckAvailability')}}</a>
                <div class="time_list mb-2">
                    <label class="pb-0">{{showLangTitle('Pleaseselecttheslot')}}</label>
                    <ul id="cont"></ul>
                </div>
            </div>
            ...
            <div class="booknow_btn ">
                <div class="price_box">
                    <p class="text-lg font-semibold">{{showLangTitle('TotalPrice')}}:</p>
                    <span :class="`heading_sm3 ${this.old_price_class}`" id="final_pay_price">{{showPrice(this.final_pay_price)}}</span>
                    <span class="heading_sm3" id="agent_price" v-if="parseInt(this.agent_price) > 0">{{showPrice(this.agent_price)}}</span>
                </div>
                <div class="price_box">
                    <input type="hidden" name="booking_price" id="booking_price" :value="this.package_booking_price">
                    <p class="text-lg font-semibold" v-if="parseInt(this.booking_price) > 0">{{showLangTitle('BookingPrice')}}:</p>
                    <span class="heading_sm3" v-if="parseInt(this.booking_price) > 0">{{showPrice(this.booking_price)}}</span>
                </div>
                <ul class="btn_group listing_btn">
                    ...
                    <li class="text-center">
                        <Link :href="`${package.enquiry_url}&type=${typename}`" class="btn2">{{showLangTitle('EnquireNow')}}</Link>
                    </li>
            ...
            <div class="note_text text-sm mb-3 pt-2"><strong>{{showLangTitle('Note*')}} : </strong> {{showLangTitle('Morethan6personspleasecontact')}}.</div>
                ...
                <Popup className="fullCalander_pop_wrap">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <span class="heading">{{showLangTitle('SelecttheDateofTravel')}}</span>
                            <span class="sub_title_pop">{{showLangTitle('ThePriceInCalendarRepresentTheStartingPricePerPerson')}}.</span>
                        </div>
        ...
        $('.right_side').find(".element_choice").each(function( index ) {
            ...
            if(is_calculate==1) {
                ...
                if(parseInt(selected_single_price_value) > 0){
                    $('.right_side').find('.price_select'+elementId).html(currentObj.showPrice(selected_single_price_value));
                } else {
                    $('.right_side').find('.price_select'+elementId).html('');
                }
                if(original_price_val != selected_price_value){
                    $('.right_side').find('.price_select'+elementId).html('<s>'+currentObj.showPrice(original_price_val)+'</s> '+currentObj.showPrice(selected_price_value));
                }
            }
        });
