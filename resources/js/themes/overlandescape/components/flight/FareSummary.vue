<template>
    ...
    <div class="styles_overlay" :class="showChangedPrice ? 'active' : ''">
        <div class="styles_modal promtbox_wrapper comform_popup">
            <div class="promtbox_wrapper_area">
                <h2 class="promtbox_wrapper-mainheading">Confirm To Proceed</h2>
                <p class="promtbox_wrapper-content">Fare have changed</p>
                <div class="content_flex">
                    <div class="flex">
                        <span>Old Fare</span>
                        <span>{{showPrice(store.bookingTotalPrice)}}</span>
                    </div>
                    <div class="arrow">
                    ...
                    <div class="flex">
                        <span>New Fare</span>
                        <span class="totalp">{{showPrice(this.totalPrice)}}</span>
                    </div>
                </div>
                <div class="promtbox_wrapper-button flex">
                    <button class="btn btn-warning asr-book" @click="goback">Back</button>
                    <button class="btn btn-warning asr-book" @click="handleContinue">Continue</button>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    created() {
        if(this.info.totalPriceInfo && this.info.totalPriceInfo.totalFareDetail.fC.TF) {
            ...
            var totalPrice = this.info.totalPriceInfo.totalFareDetail.fC.TF + supplier_markup + agent_markup - agent_discount;
            this.totalPrice = totalPrice;
            if(store?.bookingTotalPrice) {
                if(parseInt(totalPrice) > parseInt(store.bookingTotalPrice)) {
