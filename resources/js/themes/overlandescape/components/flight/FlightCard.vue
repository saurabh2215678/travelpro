<template>
       <flight-card-wrapper v-if="this.flightSegments.length > 0 && this.showFlightCard" :style="`--box-top-pos: ${cardClientInfo.top}px; --box-left-pos: ${cardClientInfo.left}px; --box-width-pos: ${cardClientInfo.width}px; --box-height-pos: ${cardClientInfo.height}px`" ref="bxRef">
            <td>
                <div class="expendelar_btn"></div>
                <div class="mnt_wrap">
            ...
            <td>
                <ul class="assd_price_list" :class="toggleContent ? 'showall' : 'showless'">
                    ...
                    <li v-if="priceList.fareIdentifier != 'SPECIAL_RETURN'" :class="(index>2)?'toggle_price':''" >
                        <div class="mob_price_chk_btn" @click="handleBookWithPriceId(priceList.id)"></div>
                        <label>
...
import { FlightCardWrapper } from './FlightCardWrapper';
...
export default {
    ...
    methods: {
        ...
        handleBookWithPriceId(priceId){
            store.bookingPassedStep= 0;
            store.bookingCurrentStep = 1;
            store.loadingName = 'iternity';
            this.priceId = priceId;
            this.$inertia.get(`/flight/itinerary/${priceId}`);
        },
        handleSizeUpdate(){
            const boxElem = this.$refs.bxRef;
            const flightResSecTopPos = document.querySelector('.flight_res_sec')?.getBoundingClientRect();
            const boxElemInfo = boxElem.$el?.getBoundingClientRect();
            const boxNewPos = {};
            boxNewPos.top = boxElemInfo.top - flightResSecTopPos.top;
            boxNewPos.left = boxElemInfo.left;
            boxNewPos.width = boxElemInfo.width;
            boxNewPos.height = boxElemInfo.height;
            this.cardClientInfo = boxNewPos;
        }
    },
    mounted(){
        const currObj = this;
        window.addEventListener('resize', function(){
            currObj.handleSizeUpdate();
        });
        ...
        const boxElem = this.$refs.bxRef;
        $(boxElem.$el).find('.expendelar_btn').click(function(){
