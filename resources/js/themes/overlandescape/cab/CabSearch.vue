    ...
    <Layout :metaTitle="metaTitle" :metaDescription="metaDescription">
        ...
        <template v-if="this.routeInfos[0]['places'] ">
           ...
           <div class="xl:container xl:mx-auto">
                                ...
                                <table class="search_result_table cab_results">
                                    <tbody>
                                        <template v-if="this.filteredSearchResult.ONWARD" v-for="cab, index in this.filteredSearchResult.ONWARD" :key="cab.id">
                                            <CabCard :id="index" :info="cab" :routeInfos="this.routeInfos" :paxInfo="this.paxInfo" priceIdName="ONWARD" :totalFlights="this.filteredSearchResult.ONWARD.length" :routeIndex="0" :tripType="tripType" :addons="addons" />
                                        </template>
                                    </tbody>
                                </table>
                            </div>
            ...
            <div class="xl:container xl:mx-auto">
            <div class="sights_overview">
            <template v-if="this?.routeInfos[0]?.sightseening.name">
                <h3 class="pb-3">{{this.routeInfos[0]['sightseening']['name']}}</h3>
                <div class="sightseen_route">
                    <span v-if="this?.routeInfos[0]?.sightseening.duration"><div class="titlep"><strong>Days</strong></div> <i class="fa-solid fa-calendar-days text-2xl text-teal-500" ></i> {{this.routeInfos[0]['sightseening']['duration']}}</span>
                    <span v-if="this?.routeInfos[0]?.sightseening.distance"><div class="titlep"><strong>Distance</strong></div> <i class="fas fa-tachometer-alt text-2xl text-teal-500" ></i> {{this.routeInfos[0]['sightseening']['distance']}} km</span>
                </div>

                <div class="sightseen-desc mb-5" v-html="this.routeInfos[0]['description']"></div>

                <div class="inclusions" v-if="this?.routeInfos[0]?.sightseening?.inclusion">
                    <i class="fa-solid fa-circle-check"></i> 
                    <span class="text-2xl text-slate-900 font-semibold dark:text-slate-300 py-3 ml-3">Inclusions</span>
                    <div v-html="this.routeInfos[0]['sightseening']['inclusion']"></div>
                </div>
                <div class="exclusions" v-if="this?.routeInfos[0]?.sightseening?.exclusion">
                    <i class="fa-solid fa-circle-xmark"></i>
                    <span class="text-2xl text-slate-900 font-semibold dark:text-slate-300 py-3 ml-3">Exclusions</span>
                    <div v-html="this.routeInfos[0]['sightseening']['exclusion']"></div>
                </div>
                <div class="terms" v-if="this?.routeInfos[0]?.sightseening?.term">
                    <i class="fa-regular fa-file-lines"></i>
                    <span class="text-2xl text-slate-900 font-semibold dark:text-slate-300 py-3 ml-3">Terms & Conditions</span>
                    <div v-html="this.routeInfos[0]['sightseening']['term']"></div>
                </div>
            </template>
        </div>
    </div>
...
export default {
   ...
   props:["destinationLists", "airportLists", "sightseeningDestinationLists", "routeList", "searchResult", "routeInfos", "tripType", "metaTitle", "metaDescription", "addons"]
}
