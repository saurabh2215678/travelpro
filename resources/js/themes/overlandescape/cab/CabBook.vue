                    ...
                    <div class="right_card">
                        ...
                        <!-- <div class="ss-img_box">
                            ...
                            <table class="table table-striped right_booking_details">
                                ...
                                <tr v-if="this?.routeInfo?.adult || this?.routeInfo?.children">
                                    <th>Total Pax</th>
                                    <td>:</td>
                                    <td>
                                        <span v-if="this?.routeInfo?.adult && this?.routeInfo?.children">{{this?.routeInfo?.adult}} Adults, {{this?.routeInfo?.children}} Child</span>
                                        <span v-else-if="this?.routeInfo?.adult">{{this?.routeInfo?.adult}} Adults</span>
                                        <span v-else-if="this?.routeInfo?.children">{{this?.routeInfo?.children}} Child</span>
                                    </td>
                                </tr>
                                <template v-if="this?.routeInfo?.selected_addons">
                                    <tr v-for="addon in this?.routeInfo?.selected_addons">
                                        <th>{{addon.name}}</th>
                                        <td>:</td>
                                        <td>Qty: {{addon.qty}}<span v-if="addon?.days > 0">, Days: {{addon.days}}</span> ({{showPrice(addon.price)}})</td>
                                    </tr>
                                </template>
                                <tr>
                                    <th>Car Type</th>
