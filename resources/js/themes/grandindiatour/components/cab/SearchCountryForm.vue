        ...
        <form class="flight_form" @submit="handleFormSubmit">
            ...
            <template v-else>
                ...
                <div class="select_from_wrap" v-if="tripType == 2">
                    <ModelListSelect :list="sightseeningDestinationLists.filter(item=>item.name)" v-model="fromCountry" @Input="(e)=>handleAirportChange(e,'from')" option-value="name" :customText="(item)=>handleCustomText(item)" option-text="city" placeholder="Where From ?" />
                        <span class="err" v-if="this.errors['fromCountryError']">{{this.errors['fromCountryError']}}</span>
                </div>
