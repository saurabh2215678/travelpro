            ...
            <tbody>
                <tr>
                    ...
                    <td class="price-list-view">
                        ...
                        <div v-if="row.publish_price && row.search_price && parseFloat(row.publish_price) > parseFloat(row.search_price)" class="cut-price text-lg leading-normal"><span class="text-slate-500 relative">{{showPrice(row.publish_price)}}</span></div>
                        <div class="ss_price text-blue-800 text-lg font-semibold">{{showPrice(row.search_price)}}</div>
