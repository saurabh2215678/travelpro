<template>
    <div class="full_loading" :class="this.processing ? 'loading' : ''" >
        <img :src="`${store.websiteSettings?.FRONTEND_ASSETS_URL}/images/loading-gear.gif`" />
    </div>
    <div class="stp_wrap">
            ...
            <div class="left_tab_content">
                <template v-if="store.userInfo && store.userInfo.balance && store.userInfo.balance >= this.totalPrice">
                    <div class="diposit_content" v-if="activeTab == 'wallet'">
                        ...
                        <div class="paynow-txt">Amount to Pay: {{showPrice(this.totalPrice)}}</div>
                        <button class="btn" :disabled="true" v-if="this.processing"> Processing...</button>
                        <button class="btn" :disabled="'wallet' != termsChecked" @click="handlePayment(`wallet`)" v-else>Pay Now </button>
                        <div class="agree_line">
                            <label>
                                ...
                                By proceeding to book, I Agree to {{store.websiteSettings.APP_NAME}} <a :href="store.websiteSettings.PRIVACY_LINK" target="_blank">Privacy Policy</a> and  <a :href="store.websiteSettings.TERMS_SERVICE_LINK" target="_blank">Terms of Service</a>
                            </label>
                        </div>
                    </div>
                </template>
                ...
                <template v-for="payment_gateway in payment_gateways">
                    <div class="diposit_content" v-if="activeTab == payment_gateway.value">
                        ...
                        <div class="paynow-txt">Amount to Pay: {{showPrice(this.totalPrice)}}</div>
                        <button :disabled="true" v-if="this.processing">Processing...</button>
                        <button class="btn" :disabled="payment_gateway.value != termsChecked" @click="handlePayment(`${payment_gateway.value}`)" v-else>Pay Now</button>
                        <div class="agree_line">
                            <label>
        ...
        handlePayment(payment_method) {
            ...
            .then(function (response) {  
                ...
                if(response.data.success) {
                    ...
                    currentObj.showErrorToast(response.data.message,3);
            ...
            .catch(function (error) {
                ...
                if(error.response.data.message) {
                    currentObj.showErrorToast(error.response.data.message,3);
                }
            });
        },
    },
