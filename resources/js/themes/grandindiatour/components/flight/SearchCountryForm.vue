...
& .passenger_wrap {
    display: block !important; top: auto; left: auto; right: auto; border-radius: 0.36rem;flex: initial; width: initial; overflow: initial; box-shadow: none; padding: 0;z-index: initial;
}
            ...
            if(value && value.length >= 3) {
                ...
                .then(function (response) {
                    ...
                    if(response.data.success) {
                        if(counter) {
                            if(origin == 'from') {
                                currentObj.multifromCountryList[counter] = response.data.items;
                            } else if(origin == 'to')  {
                                currentObj.multitoCountryList[counter] = response.data.items;
                            }
                        } else {
                            if(origin == 'from') {
                                currentObj['fromCountryList'] = response.data.items;
                            } else if(origin == 'to') {
                                currentObj['toCountryList'] = response.data.items;
                            }
                        }
