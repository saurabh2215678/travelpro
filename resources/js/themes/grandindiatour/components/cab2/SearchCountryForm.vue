...
<SearchWrapper class="book_flight_form" :class="`trip-type-${tripType}`">
    ...
    <form class="flight_form" @submit="handleFormSubmit" ref="formRef">
        ...
        <div class="trp_box"  v-if="tripType == 3 || tripType == 4">
            ...
            <div class="trip-type relative">
                <select @change="handlePickup">
                    <option value="0" :selected="this.pickupType == 0"> Pickup </option>
                    <option value="1" :selected="this.pickupType == 1"> Drop </option>
                </select>
        ...
        <div class="select_from_wrap pickup_loc" v-if="isPickupLocationShown()">
            ...
            <input type="text" name="pickup_location" class="free_input" id="places-search" placeholder="Pick Up Location" :value="selectedPickup?.name" />
        ...
        <div class="select_from_wrap dest_sel" v-if="isDestinationLocationShown()">
            ...
            <input type="text" name="destination" class="free_input" placeholder="Destination" :value="selectedDestination?.name" />
        ...
        <div class="ssb-wrap date sel_date">
            <label>Date</label>
            <DatePicker columns="2" v-model="selectedDate" mode="date" class="date_wrap" :min-date="this.departureMinDate">
                    ...
                    <select class="pickup-time" @change="this.handleTimeChange">
                        ...
                        <template v-if="this.tripTimeArr">
                            <template v-for="tripTimeRow,index in this.tripTimeArr">
                                <option :value="tripTimeRow.key" :selected="tripTimeRow.key==this.selectedTime">{{tripTimeRow.title}}</option>
                            </template>
                        </template>
                    </select>
                </div>
        ...
        <div class="ssb-wrap ret_date" v-if="tripType == 2 && returnEnabled">
            ...
            <DatePicker columns="2" v-model="returnDate" mode="date" class="date_wrap" :min-date="this.departureMinDate">
                <template v-slot="{ inputValue, inputEvents, togglePopover }">
                    <input
                    class="date_input"
                    :value="inputValue"
            ...
            <div class="passenger-economy" >
                ...
                <div class="passenger-txt">
                    <select class="pickup-time" @change="this.handleReturnTimeChange">
                        ...
                        <template v-if="this.tripTimeArr">
                            <template v-for="tripTimeRow,index in this.tripTimeArr">
                                <option :value="tripTimeRow.key" :selected="tripTimeRow.key==this.returnTime">{{tripTimeRow.title}}</option>
                            </template>
                        </template>
                    </select>
                </div>
...
export default {
    ...
    created() {
        ...
        // }
        if(store?.websiteSettings?.CAB_BOOKING_AFTER_HOURS > 0) {
            this.departureMinDate = new Date(moment(curDate).add(store.websiteSettings.CAB_BOOKING_AFTER_HOURS, 'hours'));
        } else {
            this.departureMinDate = new Date();
        }

        if(store?.searchData?.pickupType) {
            this.pickupType = store.searchData.pickupType;
        }
        if(store?.searchData?.city) {
            this.selectedCity = store.searchData.city;
            if(this.TripType == 3) {
                this.getTerminalLists(store.searchData.city.id)
            }
            if(this.TripType == 4) {
                this.getStationLists(store.searchData.city.id)
            }
        }
        if(store?.searchData?.pickup) {
            this.selectedPickup = store.searchData.pickup;
        }
        if(store?.searchData?.destination) {
            this.selectedDestination = store.searchData.destination;
        }
        if(store?.searchData?.terminal) {
            this.selectedTerminal = store.searchData.terminal;
        }
        if(store?.searchData?.station) {
            this.selectedStation = store.searchData.station;
        }
        if(store?.searchData?.dep) {
            this.selectedDate = new Date(store.searchData.dep);
        }
        if(store?.searchData?.time) {
            this.selectedTime = store.searchData.time;
        }
        if(store?.searchData?.return) {
            this.returnDate = new Date(store.searchData.return);
        }
        if(store?.searchData?.return_time) {
            this.returnTime = store.searchData.return_time;
        }
        if(store?.searchData?.returnEnabled) {
            this.returnEnabled = store.searchData.returnEnabled;
        }
    },
    data() {
        return {
    ...
    methods: {
        ...
        setActiveTab(tabType) {
            ...
            if(this.selectedCity) {
                if(tabType == 3) {
                    this.getTerminalLists(this.selectedCity.id)
                }
                if(tabType == 4) {
                    this.getStationLists(this.selectedCity.id)
                }
            }
            this.errors = {};
        },
