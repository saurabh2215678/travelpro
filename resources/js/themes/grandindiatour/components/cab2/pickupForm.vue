<template>
    <form @submit.prevent="handleSubmit" class="rts_flight">
        ...
        <div class="fts_top" ref="cabRef">
            ...
            <div class="form_item">
                <div class="input_item floating_input">
                    <input type="text" class="input_control"  placeholder="PICKUP" name="pickup" @input="handleInput" :value="routeInfo?.fromCity?.name" readonly v-if="routeInfo?.fromCity?.name && tripType != 5" >
                    <input type="text" class="input_control"  placeholder="PICKUP" name="pickup" @input="handleInput" v-else>
                    <label>PICKUP</label>
                </div>
            ...
            <div class="form_item">
                <div class="input_item floating_input">
                    <input type="text" class="input_control"  placeholder="DROP" name="drop" @input="handleInput" :value="routeInfo?.toCity?.name" readonly v-if="routeInfo?.toCity?.name && tripType != 5" >
                    <input type="text" class="input_control"  placeholder="DROP" name="drop" @input="handleInput" v-else>
                    <label>DROP</label>
                </div>
                <div class="error" v-if="this.errors?.drop">{{this.errors.drop}}</div>
            </div>

            <p class="font14"><input type="checkbox" id="" name="" value=""> By proceeding to book, I Agree to {{routeInfo.app_name}} <strong><a :href="routeInfo.privacy_link" target="_blank">Privacy Policy</a></strong> and  <strong><a :href="routeInfo.terms_service_link" target="_blank">Terms of Service</a></strong></p>
...
export default {
    ...
    data() {
        return {
            ...
            formdata : {
                pickup: this.routeInfo?.fromCity?.name,
                drop: this.routeInfo?.toCity?.name
            },
            store
        }
    },
    methods: {
        ...
        formSubmit(){
            ...
            this.formdata.price_id = this.routeInfo.price_id;
