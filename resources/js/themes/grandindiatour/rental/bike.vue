<template>
    ...
    <section class="hotel_bookinglist" v-else>
        <div class="xl:container xl:mx-auto">
            <div class="row">
                ...
                <div class="pull-right lg:w-3/4" v-if="bikeList && bikeList.length > 0">
                ...
                <div class="pull-right lg:w-3/4" v-else>
                <div class="alert_not_found">No Bikes(s) found matching your search criteria. Please look for an alternate package or call our travel experts at <a :href="`tel:${store.websiteSettings?.BOOKING_QUERIES_NO}`">{{store.websiteSettings?.BOOKING_QUERIES_NO}}.</a> You may also drop us an email at <a :href="`mailto:${store.websiteSettings?.SALES_EMAIL}`">{{store.websiteSettings?.SALES_EMAIL}}</a></div>
                </div>
            </div>
        </div>
       </section>
