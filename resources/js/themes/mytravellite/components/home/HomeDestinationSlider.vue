<template>
    <DestinationSliderWrapper class="HomeDestinationSlider">
        <div class="container">
            <div class="home_destinations">
                <DestinationSlider :destinations="destinations" :title="title"/>
            </div>
        </div>
    </DestinationSliderWrapper>
</template>
<script>
import {DestinationSliderWrapper} from '../../styles/DestinationSliderWrapper';
import DestinationSlider from './DestinationSlider.vue';
import axios from "axios";
export default{
    name : 'HomeDestinationSlider',
    created(){
        this.loadDestinations();
    },
    data(){
        return{
            destinations : []
        }
    },
    methods:{
        loadDestinations(){
            var currentObj = this;
            let form_data = {};
            form_data['featured'] = 1;
            axios.post('/destination/ajaxDestinationList',form_data)  
            .then(function (response) {
                if(response.data.success) {
                    let destinationsData = response.data?.results;
                    currentObj.destinations = destinationsData.data;
                    console.log('destinationsData>>', destinationsData);
                } else {
                    alert('Something went wrong, please try again.');
                }
            });
        },

    },
    components:{
        DestinationSliderWrapper,
        DestinationSlider
    },
    props: ['limit', 'featured', 'title', 'viewAllUrl','viewAllTitle', 'destinationType', 'isGallery'],
}
</script>