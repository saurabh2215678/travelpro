      ...
      <span class="package_tour_type_text pb-1"> {{$package_tour_type_text ?? ''}}</span>
         ...
         <div class="images">
           <img src="{{$packageThumbSrc}}" alt="{{$package_title}}" />
            ...
            @endif
             <a class="featured_content package_info px-1.5 py-3.5" href="{{route($packageDetailName,['slug'=>$package_slug])}}" >            
            <div class="title text-sm">
               {{ $package_title }}</div>
        
            @if(!empty($final_amount))
            <div class="price">
               <p class=""> Starting From : <span class="amount heading_sm3"><?php echo ($sub_total_amount == $final_amount) ? "" : "<small class='old_price'>".CustomHelper::getPrice($sub_total_amount)."</small>"; ?>{{CustomHelper::getPrice($final_amount)}}</span></p>
