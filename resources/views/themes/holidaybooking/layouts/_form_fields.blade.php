...
switch ($element->type) {
	case 'name':
	?>
	<div class="form-floating mb-3 {{$className}}"><label for="{{$name}}">{{$placeholder}}</label><input type="text" placeholder="{{$placeholder}}" {{($validation=='required')?'required':''}} class="form-control {{$classNameInner}}" dataTypeInput="input{{$element->type}}" name="input{{$id}}" value="{{old('input'.$id)}}" /><span class="input{{$id}}_error error"></span></div>
	<?php
	break;

case 'email':
?>
<div class="form-floating mb-3 {{$className}}"><label for="{{$name}}">{{$placeholder}}</label><input type="email" placeholder="{{$placeholder}}" {{($validation=='required')?'required':''}} class="form-control {{$classNameInner}}" dataTypeInput="{{$element->type}}" name="input{{$id}}" value="{{old('input'.$id)}}" /><span class="input{{$id}}_error error"></span></div>
<?php
break;

case 'phone':
?>
<div class="form-floating mb-3 {{$className}}"><label for="{{$name}}">{{$placeholder}}</label>
	...
	<input type="text" {{($validation=='required')?'required':''}} placeholder="{{$placeholder}}" class="form-control {{$classNameInner}}" dataTypeInput="{{$element->type}}" name="input{{$id}}" value="{{old('input'.$id)}}" maxlength="12" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" /><span class="input{{$id}}_error error"></span>
</div>
<?php
break;
...
case 'country':
?>
<div class="form-floating mb-3 {{$className}}"><label for="{{$name}}">{{$placeholder}}</label><select class="form-select {{$classNameInner}}" {{($validation=='required')?'required':''}} name="input{{$id}}">
	<?php
...
case 'number':
?>
<div class="form-floating mb-3 {{$className}}"><label for="{{$name}}">{{$placeholder}}</label><input type="text" {{($validation=='required')?'required':''}} placeholder="{{$placeholder}}" class="form-control {{$classNameInner}}" dataTypeInput="{{$element->type}}" name="input{{$id}}" value="{{old('input'.$id)}}" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" /><span class="input{{$id}}_error error"></span></div>
<?php
break;

case 'textarea':
?>
<div class="form-floating mb-3 {{$className}}"><label for="{{$name}}">{{$placeholder}}</label><textarea placeholder="{{$placeholder}}" {{($validation=='required')?'required':''}} class="form-control {{$classNameInner}}" dataTypeInput="{{$element->type}}" name="input{{$id}}" style="height: 100px;">{{old('input'.$id)}}</textarea><span class="input{{$id}}_error error"></span></div>
<?php
break;

	case 'text_only':
?><div class="form-floating mb-3 {{$className}}"><label for="{{$name}}">{{$placeholder}}</label><div>{{$data}}</div><span class="input{{$id}}_error error"></span></div>
<?php
break;
...
case 'checkbox':
		...
		//w-100 labelasHead light_content For Disclaimer
			if(!empty($data)){ //col-md-4 ?>
				<div class=" {{$className}}">
					<div class="form-floating checkbox_wrapper mb-3"><label for="{{$name}}">{{$placeholder}}</label><?php $checkboxtData = explode('#', $data);
	...
	<span class="input{{$id}}_error error"></span>
		...
		//pr($checkedVal);
		if(!empty($radioData)){
			?><div class="form-floating radio_wrapper w-100 mb-5 pb-4 mt-4 pt-2 {{$className}}"><label for="{{$name}}">{{$placeholder}}</label><?php 			
...
case 'file':
?>
<div class="w-100 file_wrapper {{$className}}"><label for="{{$name}}">{{$placeholder}}</label>
...
case 'url':
?>
<div class="form-floating mb-3 {{$className}}"><label for="{{$name}}">{{$placeholder}}</label><input type="url" placeholder="{{$placeholder}}" {{($validation=='required')?'required':''}} class="form-control {{$classNameInner}}" name="input{{$id}}" value="{{old('input'.$id)}}" /><span class="input{{$id}}_error error"></span></div>
<?php
break;
