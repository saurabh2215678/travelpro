...
<form action="{{ route('packageListing') }}" method="GET" name="package_search">
   <div class="enquiry_search">
      ...
      <section class="destination-slider <?php if($popular_packages->isEmpty()){ echo 'empty_dest'; } ?>" style=" padding-top: 30px; ">
         <div class="container">

            <h1 class="font30 fw600 color_secondary text_center mb-6" >Places to Visit in {{$destination_name}}</h1>
            <div class="theme_title mb-4">
               <h2 class="font22 fw600 mb-0">Top {{$destination_name}} Tours</h2>
               <a class="arrow_btn" href="{{route('tourPackages',['slug'=>$destination_slug])}}">View All <img src="{{ asset(config('custom.assets').'/images/right_arrow.svg') }}" alt=""></a>
            </div>
...
<?php if(!$popular_activities->isEmpty()){ ?>
  <section class="destination-slider">
    <div class="container">
      ...
      <div class="theme_title mb-4">
         <h2 class="font30 fw600 color_secondary mb-0">{{$destination_name}} Tour Activity</h2>
         <a class="arrow_btn" href="{{route('tourActivities',['slug'=>$destination_slug])}}">View All <img src="{{ asset(config('custom.assets').'/images/right_arrow.svg') }}" alt=""></a>
      </div>
      ...
      <div class="container">
         <div class="tab">
          ...
          if(isset($destination_type) && !empty($destination_type) > 0)
          {
            foreach($destination_type as $info)
            {
               ...
               $info_slug = isset($info->slug) ? $info->slug:$info_title;
               //prd($info_slug);
         ...
         <?php if(isset($destination_type) && !empty($destination_type) > 0) {
          foreach($destination_type as $info) {
            ...
            ?>
            <div id="{{$info->slug??$info_title}}" class="tabcontent">                 
        ...
        @if(!empty($destinationImages) && count($destinationImages)>0)
          <div id="dest_gallery" class="tabcontent">
            <div class="flex flex-wrap">
               ...
               <a href="{{asset('/storage/'.$destination_path.$image->name)}}" data-fancybox="destination-gallery"><img src="{{asset('/storage/'.$destination_thumb_path.$image->name)}}" alt="{{$image->title??''}}"></a>
               @endforeach
            </div>
          </div>
          @endif
    ...
    <div class="container">
      <div class="box_style testimonials_inner">
        ...
        <div class="testimonial_list_outer">
          ...
          $testimonialGalleryDefaultImage =asset(config('custom.assets').'/images/user.png');
...
<?php if(!empty($faq_row) && count($faq_row) > 0){
   ...
   <div class="secpad inner-page faq faq-cms-block">
      ...
      <div class="theme_title mb-6">
         <div class="title text-2xl">FAQs About {{$destination_name??''}}</div>
      </div>
        <?php $i = 1; ?>
        <div class="faqlist">
         <ul>
