...
.monthly-wrp table button:hover { background-color: #3e1412; }
 ...
 .tbl_left { width: calc(100% - 20rem); padding-right: 2rem; }
 .tbl_right { width: 20rem; }
 .tbl_right .form_box_inner { /*position: sticky; top: 6rem;*/ }
 li.faq_main li { font-weight: 400; font-size: 1rem; font-family: 'SF-Pro-Display'; }
 #payment-policy .faq_main ul, #cancellation-policy .faq_main ul { list-style: decimal; padding-left: 1rem; }
 .form-floating.mb-3 { margin-bottom: 0.1rem!important; }
 .header_inner_page .theme_header{z-index: 999999;}
 [data-popup-modal].active{opacity: 1!important;}
 .backdrop { z-index: 999999!important; }
 div#viewFullCalModal{z-index: 9999999;}
 [data-popup-modal]{pointer-events: all!important;}
 #hotel_accom>h3 { font-size: 1.313rem; font-weight: 500!important; }
 #hotel_accom>h3+.pb-2 { display: none; }
...
?>
<style>
   div#viewFullCalModal{display: flex !important; transition: 0.5s!important;}
   .modal.fade .modal-dialog { transform: none!important; }
   .modal_opened { overflow: hidden; padding-right: 16px;}
   .modal-content .modal-header { flex-direction: column; align-items: flex-start; }
   .modal-content .modal-header button#hmodel-close { display: grid; place-items: center; }
   [data-popup-modal]{visibility: visible!important; opacity: 0!important; pointer-events: none!important;}
   [data-popup-modal].active{pointer-events: all!important;}
   .backdrop{opacity: 0!important;display: block!important; pointer-events: none!important; transition: 0.5s;}
   .backdrop.active{pointer-events: all!important; opacity: 1!important;}

   div#viewFullCalModal *{pointer-events: none!important;}
   div#viewFullCalModal.active *{pointer-events: all!important;}
   #myForm .booknow_btn .btn_group input.btn{display: none!important;}
   .price_box:first-child+.price_box { display: none!important; }
   .accordion.accomodation_acc { display: flex; }
   .hodel_detail_list { display: flex; }
   .hotel_info { box-shadow: 0 0 9px #0000001a; margin: 5px; border-radious:5px; overflow:hidden;}
   .hotel_info .hotel_info_box.itenery_info{margin-top: 0!important; padding-bottom: 0!important;}
   .hotel_info_box .rating_list { padding: 0; }
   .hotel_info .hotel_destination { padding: 1rem; padding-bottom: 0.5rem; }
   .hotel_info .box-content { padding: 0 1rem; }
   body.no-accomodation .accom_btn { display: none; }
   body.no-accomodation #package_accommodations{display:none;}
   #package_accommodations { margin-bottom: 2.6rem; }
   form#form_download_itinerary .form-floating>label {
    position: static;
    padding: 8px 0 0;
}
.form_box .btn-default.show-modal {
    background: var(--theme-color);
    color: #fff;
    padding: 5px 10px;
}
.form_box .btn-default.show-modal:hover {
...
<div class="row m-0">
   <div class="container mx-auto">
      ...
      <div class="tbl_wrap">
         <div class="tbl_left">
            <div class="tab_package_detail">
               @if($package->brief!='')
               <a href="#about_link" class="tablinks1">Overview</a>
               @endif

               @if($package->brief!='')
               <a href="#package_accommodations" class="tablinks1 accom_btn">Accomodation</a>
               @endif

               @if(isset($package->inclusions) && !empty($package->inclusions))
               <a href="#inclusion" class="tablinks1">Inclusions & Exclusions</a>
               @endif

               @if(isset($package->exclusions) && !empty($package->exclusions))
               <!-- <a href="#exclusions" class="tablinks1">Exclusions</a> -->
               @endif

               @if(!empty($PackageInfo) && count($PackageInfo) > 0)
               <a href="#other-policy" class="tablinks1">Other Policies</a>
               @endif

               @if(!empty($package->payment_policy))
               <!-- <a href="#payment-policy" class="tablinks1">Payment Policy</a> -->
               @endif

               @if(isset($package->cancellation_policy) && !empty($package->cancellation_policy))
               <!-- <a href="#cancellation-policy" class="tablinks1">Cancellation Policy</a> -->
               @endif

               @if(isset($package->terms_conditions) && !empty($package->terms_conditions))
               <!-- <a href="#terms-conditions" class="tablinks1">Terms Conditions</a> -->
               @endif
            ...
            <div class="tabcontent1">
               ...
               <div class="overview_box" id="about_link">
                  @if($package->brief!='')
                  <p>{!!$package->brief!!} 
                     @if($package->description!='')        

                     <div class="para_lg2" id="h_dec" style="display: none">{!!$package->description!!}</div>
                     ...
                     <?php if(isset($package_itenaries) && !empty($package_itenaries) && $package->is_activity==0){ ?>
                        <div class="package_day secpad mt-5">
                           <div class="container-full">
                              <h3 class="font21">Itinerary</h3>
                              <div class="faqlist faqlist_in mb-5">
                                 <ul>
                                    ...
                                    // prd($itenary_inclusions_arr);
                                       ...
                                       <li class="faq_main">
                                          <div class="ml-14">
                                             ...
                                             @if($itenary_location)
                                             <div class="desti">{{$itenary_location}}</div>
                                             @endif
                                             @if(!empty($itenaryTags))
                                          <?php /* <div class="theme_tags mb-5">
                                         ...
                                         @if(!empty($meal_option_arr))  
                                          ...
                                          <div class="day_curcle">{{ $itenary_day_title }}<span></span></div>
                                          <div class="faq_data">
                                             @if(!empty($itenary_inclusions_arr))
                                             <div class="faq_service heading6 mb-3">
            ...
            <?php if((isset($package->inclusions) && !empty($package->inclusions)) || (isset($package->exclusions) && !empty($package->exclusions)) || (isset($package->payment_policy) && !empty($package->payment_policy)) || (isset($package->cancellation_policy) && !empty($package->cancellation_policy)) || (isset($package->terms_conditions) && !empty($package->terms_conditions))){ ?>

               <div class="faqlist bg_inclsn_tab">

                  @if(isset($package->inclusions) && !empty($package->inclusions))
                  <ul class="p-0" id="inclusion">
                   <li class="faq_main">
                    <div>
...
<?php if(!empty($faq_row) && count($faq_row) > 0){
   ...
   <div class="secpad inner-page faq faq-cms-block">
      <div class="container" itemscope itemtype="https://schema.org/FAQPage">
         <div class="mb-3 mt-5">
            <h3 class="no_line text-xl pb-1 font-bold">FAQs About {{$package->title}}</h3>
         </div>
         <?php $i = 1; ?>
         <div class="faqlist">
            <ul class="p-0">
               ...
               <li class="faq_main <?php $i==1 ? 'active':''?>">
                  <div itemscope itemtype="https://schema.org/Question" itemprop="mainEntity">
                     <div itemprop="name" class="faq_title heading6"><span><strong>Q{{$i}}. </strong></span>{{$qusArr}}</div>
...
<div class="flex flex-wrap package_brief d-none">
   <div class=" w-70 pr5">
...
<?php if(!$testimonials->isEmpty()){ ?>
   <section class="home_testimonial package_testi">
     <div class="container">
                  ...
                  <div class="testimonial_box">
                     <div class="client_info">
                      <a href="{{ route('testimonialDetails',['slug'=>$tSlug]) }}">
                        ...
                        <div class="client_info">
                           <div class="name para_lg2 w-full">{{ $tName }}</div>
                           <div class="city">{{ $tLocation }}</div>
                           <div class="font-normal">{!! $tDescription !!}</div>
                        </div>
                     </a>
                  </div>
...
<?php if(!empty($relatedPackagestObj) && !($relatedPackagestObj)->isEmpty()){ ?>
   <section class="accommodation_single">
      <div class="container">
         <div class="text_center">
            <div class="theme_title mb-3">
               <div class="title text-2xl	">Related {{$is_activity==1?'Activity':'Packages'}}</div>
            </div>
         </div>
         <div class="slider_box ">
...
<?php if(!$destinations->isEmpty()){ ?>
   <section class="departure_single d-none">
      <div class="container">
         <div class="text_center">
      ...
      $.ajax({
        ...
        success: function(response) {
          if(response.success) {
           ...
           if($('.accomodation_acc').length <= 0){
            $('body').addClass('no-accomodation');
         }
...
<script type="text/javascript">
   ...
   $("header").addClass("header_bottom_line");
...
$(document).click(function(e){
   if($('#viewFullCalModal').hasClass('active')){
   ...
   if(element.closest('.modal-dialog').length > 0 || (element.attr('data-popup-btn') == 'departure-date')){
         // console.log('inside click')
   }else{
      $('body').removeClass('modal_opened');
      $('#viewFullCalModal').removeClass('active');
