...
<section class="p-0">
  <div class="home_banner">
    <img src="{{ $b_image }}" class="w-100" alt="{{$page_title??''}}">
  </div>
</section>
...
<div class="xl:container xl:mx-auto">
      ...
      <div class="theme-title">
         ...
         <div class="team-grid_outer">
            <ul class="team-grid grid lg:grid-cols-4 grid-cols-4 gap-4">
...
<?php foreach ($teams as $teamLists) {
            ...
            $manager_sub_title = (!empty($teamLists->sub_title)) ?($teamLists->sub_title): '';
            $manager_designation = (!empty($teamLists->designation)) ?($teamLists->designation): '';
            $mobile_no = !empty($teamLists->mobile_no)?$teamLists->mobile_no:'';
            $phone_no = CustomHelper::getPhoneHref($mobile_no);
            $email = !empty($teamLists->email)?$teamLists->email:'';
            $team_email = CustomHelper::getEmailHref($email);
            $manager_slug = $teamLists->slug;
         ...
         ?>
               ...
               <div class="content-box">
                     ...
                     <p class="pb-3 bold">{{ $manager_designation }}</p>
                     
                     @if(!empty($mobile_no))
                     <p class="phone"><img src="{{ asset(config('custom.assets').'/img/telephone.png') }}" alt="Telephone">{!! $phone_no !!}</p>
                     @endif

                     @if(!empty($email))
                        <p class="pb-3 email"><img src="{{ asset(config('custom.assets').'/img/mail.png') }}" alt="Email">{!! $team_email !!}</p>
                     @endif
...
// For toggle
// var elem = document.querySelector('#myCard');
