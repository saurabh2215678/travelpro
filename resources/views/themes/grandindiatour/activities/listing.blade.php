...
<section class="packaging_wrap p_list_view pt-11">
   <div class="xl:container xl:mx-auto">
      <div class="packaging_wrap_inner">
            <div class="side_bar">
      <form action="{{ route('activityListing') }}" method="GET" id="adv_package_search" name="adv_package_search">
         ...
         <div class="packaging_view">
               ...
               <div class="sort_by">
                  ...
                  <?php if(!$packages->isEmpty()){
                     foreach ($packages as $package) {
                        ...
                        $package_tour_type = $package->tour_type ? $package->tour_type : 'fixed';
                  ...
                  <a href="{{ route($packageDetailName,['slug'=>$package_slug]) }}"><img src="{{ $packageThumbSrc }}" class="theme_radius img_responsive" alt="{{ $package_title }}"></a>
                        ...
                        <div class="packaging_info">
                           ...
                           <span class="package_tour_type_text">{{$package_tour_type_text ?? ''}}</span>
                              <a class="py-2" href="{{ route($packageDetailName,['slug'=>$package_slug]) }}">{{ $package_title }}</a></div>
                           <div class="flex flex-wrap flex-start">
                              @if($package->is_activity==1)
                              <div class="duration mb-3 pr-5 text-sm"><i class="fa fa-clock-o" aria-hidden="true"></i> {{ $package_duration }}</div>
                              <div class="location mb-3 text-sm"><i class="fa fa-map-marker" aria-hidden="true"></i> {{$package->packageDestination->name??''}}</div>
                              @else
                              <div class="duration mb-3">{{ $package_duration }}</div>
                              @endif
                           </div>
                        ...
                        <div class="price">
                           ...
                           <div class="price_inner mb-2">
                              ...
                              <span class="amount heading_sm3"><?php echo ($sub_total_amount == $final_amount) ? "" : "<small class='old_price'>".CustomHelper::getPrice($sub_total_amount)."</small>"; ?>{{CustomHelper::getPrice($final_amount)}}</span>
                              <div class="peerson"><?php //<p class="text-sm">( {{$priceCategoryDetails->display_title??''}} )</p> ?></div>
                           </div>
                           @endif
                        ...
                        <div class="right_side flex flex-wrap my-3">
                           <a href="{{ route($packageDetailName,['slug'=>$package_slug]) }}" target="_blank" class="btn mr-3 text-sm rounded-full">Book Now</a> <!-- 'service_level'=>$package_service_level -->
                              ...
                              <div class="package_type">  
                                    <?php foreach($packagePrices as $price){ ?>
                                       <div class="customradio ">
                                          <input type="radio" name="package_type_{{$package->id}}" value="{{$price->id}}" class="mr-2" data-package_id="{{$package_id}}" {{($price->is_default==1)?'checked':''}} >{{$price->title}}
                                       </div>
                                    <?php } ?>                                 
                              </div>
                           <?php } ?>
            ...
            <?php }else{ ?>
               <div class="alert_not_found">No {{$is_activity==1?'Activities':'Packages'}} found matching your search criteria. Please look for an alternate package or call our travel experts at <a href="tel:
...
<script type="text/javascript">
   ...
   $("body").on('click','.package_type input[type=radio]',function() {
      ...
      var btnUrl = new URL($(this).closest('.packaging_info').find('.enquir_now').attr('href'));
      ...
      $.ajax({
         ...
         success:function (response) {
            if(response.success) {               
               ...
               _this1.closest('.packaging_info').find('.enquir_now').attr('href', finalurl);
