...
.fancybox-wrap.fancybox-type-html {max-width: 75rem; width:100% !important;left: 0 !important;   right: 0 !important; margin: 0 auto;}
   ...
   <?php
     ...
     $discount_amount = 0;
      ...
      if($is_agent == 1){
         ...
         if($totalPrice > 0){
          $discount_result = CustomHelper::getDiscountData('activity_listing',$module_type_id,$totalPrice,$group_id);
      ...
      <div class="container mx-auto">
         <div class="flex flex-wrap">
            <div class="row-cols-2 w-70 pr-9">
               ...
               <div class="text-xl font-semibold pb-3">Overview</div>

               <p>{!!$package->brief!!}

                  @if($package->description!='')
                   <div class="text-xl font-semibold pb-3">Activity Details</div>
                   {!!$package->description!!}
            ...
            @endif
               ...
               <!--- ======== Accommodation  Old ======== -->
               <?php if( (isset($package->inclusions) && !empty($package->inclusions)) || (isset($package->exclusions) && !empty($package->exclusions)) || (isset($package->payment_policy) && !empty($package->payment_policy)) || (isset($package->cancellation_policy) && !empty($package->cancellation_policy)) || (isset($package->terms_conditions) && !empty($package->terms_conditions)) || count($PackageInfo) > 0 || count($faq_row) > 0 ||  !empty($video_link) ){ ?>
               <div class="accommodation accordion">
                  <div class="container1">
                     <h3 class="text-xl font-semibold pb-3">Policy</h3>
                     <div class="faqlist">
                        ...
                        */ ?>
                        ...
                        @if(!empty($faq_row) && count($faq_row) > 0)
                            <div class="theme_title">
                                <h3 class="text-xl font-semibold pb-3">FAQs About {{$package->title}}</h3>
                            </div>
                        <?php $i = 1; ?>
            ...
            <div class="row-cols-2 w-30 mt-5">
               <div class="form_box-new single_package_form">
                  <div class="form_box_inner">
                     <div class="text-xl font-semibold">Your Preference</div>
...
<section class="home_testimonial package_testi m-5">
 <div class="container">
   <div class="theme-title text-center mb-1">
      <div class="text-xl font-semibold pb-3">Testimonials</div>
   </div>
...
<section class="accommodation_single mt-3">
   <div class="container">
      <div class="text_center">
      <div class="theme_title mb-0">
         <div class="text-xl font-semibold pb-3">Similar {{$is_activity==1?'Activity':'Packages'}}</div>
      </div>
      </div>
      <div class="slider_box">
...
<section class="departure_single">
   ...
   <div class="text_center">
      <div class="theme_title mb-0">
         <div class="text-xl font-semibold pb-3">More Trips Departure</div>
      </div>
      </div>
...
<script>
  ...
  $(document).on('click',function(e){
    ...
    if(!(clickedElement.closest('.single_package_white').length > 0 || clickedElement.closest('.select_trav').length > 0 || clickedElement.hasClass('single_package_white') )){
      $('.right_side').find(".single_package_white").removeClass('active');
    }
  });

  $(document).on('click','.select_trav',function(){
    $('.right_side').find(".single_package_white").toggleClass('active');
  });

  $(document).on('click','.right_side #applyTravellers',function(e){
    e.preventDefault();
    e.stopPropagation();
    travellerSelection();
    $('.right_side').find(".single_package_white").removeClass('active');
  ...
  $(document).on('click','.right_side #applyTripTravellers',function(e){
    e.preventDefault();
    e.stopPropagation();
    ...
    $.ajax({
      type        : "POST",
      url         : '{{url("package/".$package_id."/ajaxPriceSlots")}}',
      cache       : false,
      data        : {price_id,trip_date,total_pax},
