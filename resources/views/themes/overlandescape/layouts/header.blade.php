...
$all = Auth::user()->agentInfo??'';
$is_allowed_offline_flight_inventory = 0;
$name = '';
if($is_agent == 1 && CustomHelper::isAllowedModule('agentuser')) {
   ...
   if(CustomHelper::isOnlineBooking('flight')) {
      $is_allowed_offline_flight_inventory = $userDetails->agentInfo->is_allowed_offline_flight_inventory??0;
   }
} else {
   $name = Auth::user()->name??'';
}
   ...
   <div class="md:container">
               ...
               <ul class="contact_info">
                  ...
                  <li class="px-2.5	">
                     <a href="mailto:{{$websiteSettingsArr['SALES_EMAIL'] ?? ''}}">
                        <?php //<i class="mailicon"></i> ?>
                        <span> {{$websiteSettingsArr['SALES_EMAIL'] ?? ''}}</span>
                     </a>
                  </li>
                  @endif
      ...
      <header class="theme_header">
         ...
         <div class="theme_nav_search flex items-center">
            ...
            @else
            @if($is_allowed_offline_flight_inventory==1)
            <div class="topright">
              <ul class="top-nav">
                  <li class="dropdown">
                      <div class="parent">
                          <a href="{{route('user.myFlightBooking',['order_type'=>'pending','range'=>'all'])}}">
                              <div class="number">{{CustomHelper::getUserPendingBookingCount($userId)}}</div>
