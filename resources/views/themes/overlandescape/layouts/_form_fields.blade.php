...
switch ($element->type) {
	...
	$name_value = old('input'.$id,$name_value);
	?>
	<div class="form-floating mb-3 {{$className}}"><label>{{$name}}</label><input type="text" placeholder="{{$placeholder}}" {{($validation=='required')?'required':''}} class="form-control {{$classNameInner}}" dataTypeInput="input{{$element->type}}" name="input{{$id}}" value="{{$name_value}}" /><span class="input{{$id}}_error error"></span></div>
	<?php
	break;

case 'email':
$email_value = $user_email;
$email_value = old('input'.$id,$email_value);
?>
<div class="form-floating mb-3 {{$className}}"><label>{{$name}}</label><input type="email" placeholder="{{$placeholder}}" {{($validation=='required')?'required':''}} class="form-control {{$classNameInner}}" dataTypeInput="{{$element->type}}" name="input{{$id}}" value="{{$email_value}}" /><span class="input{{$id}}_error error"></span></div>
<?php
break;

case 'phone':
$phone_value = $user_phone;
$phone_value = old('input'.$id,$phone_value);
?>
<div class="form-floating mb-3 {{$className}}"><label>{{$name}}</label>
	...
	<input type="text" {{($validation=='required')?'required':''}} placeholder="{{$name}}" class="form-control {{$classNameInner}}" dataTypeInput="{{$element->type}}" name="input{{$id}}" value="{{$phone_value}}" maxlength="12" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" /><span class="input{{$id}}_error error"></span>
</div>
<?php
break;
