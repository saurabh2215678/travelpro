...
if($blogDetails->type == 'news') { 
   $blogUrl = route('newsDetail',['slug'=>$blogDetails->slug]);
}else {
   ...
   <section>
      ...
      <div class="breadcrumb_full">
            <div class="container">
               ...
               <li><a href="{{$urlList}}">{{$breadcrumb_title}}</a>
                  </li>
                        ...
                        <div class="single_blog_container">
                           ...
                           Categories:  <?php if ($blog_category_data) {  
                              foreach ($blog_category_data as $key => $blog_category) {
                                 ...
                                 if($blog_category->type == 'news'){
                                    $blogsListing = route('newsListing',[$blog_category->slug]);
                                 }
                              ?>
                              <a href="{{$blogsListing}}"> {{$blog_category->name ?? ''}}</a>                            
                           <?php } } ?>
                           </div>
                           
                           <div class="blog_full_content">
                           {!! CustomHelper::parseOutput($content) ?? '' !!}
                           </div>
                     ...
                     <div class="relatedblog">
                        <div class="text_center">
                           <div class="theme_title">
                              <div class="title text-2xl">Similar {{$title}}</div>
                           </div>
                        </div>
                           ...
                           <div class="swiper featured_slider">
                              <div class="swiper-wrapper">
                                 <?php foreach ($relatedBlogsObj as $rblogs) {
                                    ...
                                    if($rblogs->type == 'news') { 
                                       $blogsDetailUrl = route('newsDetail',['slug'=>$rblog_slug]);
                                    }else {
                                       $blogsDetailUrl = route('blogsDetail',['slug'=>$rblog_slug]);
                                    }
                                    ?>
                                 <div class="swiper-slide">
                                    <a class="featured_box" href="{{$blogsDetailUrl}}">
                                       <div class="images">
...
var featuredSlider =  new Swiper(".pack_slider", {
           slidesPerView: 3,
           spaceBetween:20,
           loop: false,
           speed:1000,
           navigation: {
           prevEl: ".pack_slider-prev",
           nextEl: ".pack_slider-next",
       },
         breakpoints: {
            0: {
               slidesPerView: 1,
