  ...
  <div class="col-md-12">
    <div class="bgcolor">
      ...
      <div class="page_btns">
        ...
        <a <?php if($city_id == $bike_city->id){ echo 'class="active"';} ?> href="{{route('admin.bike_master.inventory',[$bike_city['id']])}}" title="Bike City Inventory">{{$bike_city->name}}</a>
        <?php } ?>
      </div>
    </div>
  </div>
...
<div id="">
                      ...
                      <div class="dateselect"><span class="btn_date"><i class="fa fa-calendar"></i><input type="text" name="bulkdaterange" value="" /></span>
                        ...
                        <table class="table table-borderless">
                          <tbody class="date_input_box">
                            <tr>
                              <?php foreach ($bike_arr as $key => $bike) { ?>
                            ...
                            <tr>
                              ...
                              <td style="width: 100px;" class="date">
                                <input type="text" class="form-control bulk_cab_inventory" id="" placeholder="" style="width: 80px;margin: 0 auto;" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" name="bike_inventory[{{$bike}}]">
                              </td>
                              <?php } ?>
                            </tr>
                          </tbody>
  ...
  <div class="dateRow">
    <form method="POST" action="" accept-charset="UTF-8" role="form" id="bike_inventory" onSubmit="return validateInventory()" >
...
<script type="text/javascript">
  ...
  function validateBulkInventory() {
    ...
    $.ajax({
      url: "{{route($routeName.'.bike_master.bulk_inventory',['city_id'=>$city_id]) }}",
      type: "POST",
  ...
  function validateInventory() {
    var formID = 'bike_inventory';
    var _token = '{{ csrf_token() }}';
    var boxText = $('#'+formID).find('.btnSubmit').html();
    $.ajax({
      url: "{{route($routeName.'.bike_master.saveAllInventory',['city_id'=>$city_id]) }}",
      type: "POST",
      data: new FormData($('#'+formID)[0]),
      processData: false,
      contentType: false,
  ...
  $(document).on('keyup','.bike_inventory',function(e){
    var formID = 'bike_inventory';
    $('#'+formID).find('.btnSubmit').removeAttr('disabled');
  ...
  function loadInventory(date) {
    ...
    $.ajax({
      url: "{{route($routeName.'.bike_master.nextInventory') }}",
      type: "POST",
      data: formData,
      dataType:"JSON",
      headers:{'X-CSRF-TOKEN': _token},
