  ...
  <?php
  $active_menu = "accommodations".$accommodation_id.'/agent_price';
  ?>
  ...
  <div class="centersec">
    <div class="bgcolor">
      ...
      <div class="table-responsive" id="discount_form" style="display: none;margin-bottom: 10px;">
        <div class="full">
          <label style="display: inline-block;">Select discount category for this accommodation:</label>
          <select name="discount_category" id="discount_category" class="form-control admin_input1" style="display: inline-block; width:auto !important; margin-bottom:0;">
            ...
            <?php foreach ($discount_categories as $key => $discount_category) { ?>
              <option value="{{$discount_category->id ?? ''}}" {{ ($discount_category_id == $discount_category->id)?'selected':'' }}>{{$discount_category->name ?? ''}}</option>
            <?php } ?>
        ...
        <div class="col-md-6" id="update_section">
          ...
          <a href="{{route($routeName.'.accommodations.agent_price',['accommodation_id'=>$accommodation_id])}}" class="btn_admin2">Cancel</a>
        </div>
      </div>
      ...
      <table class="table table-striped table-bordered table-hover">
        ...
        if((!empty($discount_module_to_group) && count($discount_module_to_group) > 0) || $discount_category_id == '-1' ){ ?>
          ...
          foreach($rooms as $room){ 
            ...
            $publish_price = CustomHelper::getAccommodationRoomPublishPrice($room);
            ?>
            <tr>
  ...
  <script type="text/javaScript">
    ...
    function save_discoount_category() {
      ...
      $.ajax({
        url: "{{ route('admin.accommodations.add_agent_price') }}",
        type: "POST",
        data: { discount_category:discount_category, accommodation_id:accommodation_id },
        dataType:"JSON",
