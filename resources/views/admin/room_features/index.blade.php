@component('admin.layouts.main')
    ...
    @slot('title')
        Admin - Manage Room Features & Amenities- {{CustomHelper::WebsiteSettings('SYSTEM_TITLE')}}
    @endslot
...
<!-- Start Search box section     -->
   ...
   <div class="top_title_admin">
    <div class="title">
    <h2>Manage Room Features ({{ $roomfeatures->total() }})</h2>
    </div>
    <div class="add_button">
        @if(CustomHelper::checkPermission('all_masters','add'))
    <a href="{{ route($routeName.'.accommodations.roomfeature_add').'?back_url='.$back_url }}" class="btn_admin"><i class="fa fa-plus"></i>Add Room Feature </a>
    @endif
    </div>
    </div>
    <div class="centersec">
                ...
                <form class="form-inline" method="GET">
                   ...
                   <!--  <div class="clearfix"></div> -->
                    <div class="col-md-6"><br>
                        ...
                        <a href="{{url('admin/accommodations/room_feature')}}" class="btn_admin2">Reset</a>
                    </div>
                </form>
            </div>
        </div>
...
<!-- End Search box Section -->
                    ...
                    <table class="table table-bordered">
                        ...
                        <tr> 
                            ...
                            <td>
                                ...
                                <a href="{{route($routeName.'.accommodations.room_feature_view',[$roomFea['id'], 'back_url'=>$back_url])}}" title="View"><i class="fas fa-eye"></i></a>
                                @endif

                                @if(CustomHelper::checkPermission('all_masters','edit'))
                                <a href="{{ route($routeName.'.accommodations.roomfeatured_edit', $roomFea->id.'?back_url='.$back_url) }}" title="Edit Room Feature"><i class="fas fa-edit"></i></a>
                                @endif

                                @if(CustomHelper::checkPermission('all_masters','delete'))
                                <a href="javascript:void(0)" class="sbmtDelForm"  id="{{$roomFea->id}}" title="Delete Room Feature"><i class="fas fa-trash-alt"></i></i></a>

                                 <form method="POST" action="{{ route($routeName.'.accommodations.roomFea_delete', $roomFea->id) }}" accept-charset="UTF-8" role="form" onsubmit="return confirm('Do you really want to remove?');" id="delete-form-{{$roomFea->id}}">
                                    {{ csrf_field() }}
                                    {{ method_field('POST') }}
            ...
            ?>
            <div class="alert alert-warning">No Room Feature found.</div>
            <?php
        }
            ?>
  ...
  $(function() {
    $('.toggle-class').change(function() {
        ...
        $.ajax({
            type: "GET",
            dataType: "json",
            url: '{{ route($routeName.".accommodations.ChangeUserStatus") }}',
