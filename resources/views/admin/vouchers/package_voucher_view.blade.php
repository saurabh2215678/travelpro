...
.modal-open .modal .modal-content .modal-header button.close {
 ...
 <?php
   ...
   $flight_data = $OrderServiceVoucher->flight_data ? json_decode($OrderServiceVoucher->flight_data ) :[];
   ...
   $exclusion = $package_data->exclusion ?? '';
   ...
   <div class="bgcolor service-vaucher">
      ...
      <div class="row">
         ...
         <form style="padding:25px 120px;padding-top: 0;" method="POST">
           {{ csrf_field() }}
            <div class="form-title row">
               <div class="col-sm-6 col-form-label font-weight-bold"><h4>Package Voucher</h4></div>
               <div class="col-sm-6 font-weight-bold">
               <div class="text-right" style="margin-top: 10px;">
                @if(CustomHelper::checkPermission('packages','edit_voucher'))
               <a href="<?php echo route('admin.voucher.package', [$order_id,'order'=>$payment_status]) ?>" title="Package Voucher Edit" class="btn btn_admin cab_view_fancy"> Edit Voucher </a>
               @endif

               @if(CustomHelper::checkPermission('packages','view_voucher'))
               <a href="javascript:;" title="Package Voucher" class="btn sent_email btn_admin2" id="cust_btn"> Send Mail </a>
               @endif

              @if(CustomHelper::checkPermission('packages','view_voucher'))
              <a href="<?php echo route('admin.voucher.package_voucher_pdf',[$order_id]) ?>" title="Package Voucher PDF" class="btn btn-success cab_view_fancy">Download PDF </a>
              @endif
         </div>
               </div>
            </div>
            ...
            <div class="form-group row">
             <label for="input" class="col-sm-4 col-form-label">Name of the Pax</label>
               <div class="col-sm-8">
                  {{$name_of_pax}}
               </div>
            </div>
            ...
            <div class="form-group row">
             <label for="input" class="col-sm-4 col-form-label">Pax Contact</label>
               <div class="col-sm-8">
                  {{$pax_contact}}
               </div>
            </div>
          <div class="form-group row">
             <label for="input" class="col-sm-4 col-form-label">Local Contact</label>
               <div class="col-sm-8">
                  {{$local_contact}}
               </div>
            </div>
          <div class="form-group row">
             <label for="input" class="col-sm-4 col-form-label">Agent Contact</label>
               <div class="col-sm-8">
                  {{$agent_contact}}
               </div>
            </div>
            ...
            <div class="form-group row">
               <label for="input" class="col-sm-4 col-form-label">Package Address </label>
               <div class="col-sm-8">
                 {{$address}}
               </div>
            </div>
             <div class="form-group row">
               <label for="input" class="col-sm-4 col-form-label">Vehicle Types </label>
               <div class="col-sm-8">
                 {{$vehicle_type}}
               </div>
            </div>
        ...
        <?php if($flight_data && count($flight_data) > 0){  ?>
                ...
                <div class="form-group row">
                 <div class="col-sm-12">
                    <table class="table table-bordered">
                        ...
                        <?php foreach ($flight_data as $key => $flight_row) { ?>
                            <tbody>
                                <tr>
                                    <td>{{$hotel_row->dot ?? ''}}</td>
                                    <td>{{$hotel_row->flight ?? ''}}</td>
                                    <td>{{$hotel_row->sector ?? ''}}</td>
                                    <td>{{$hotel_row->departures ?? ''}}</td>                                
                                </tr>
                            </tbody>
                        <?php } ?>
                    </table>
        ...
        <div class="clearfix"></div>
            ...
            <h3>Fare Details</h3>
            <div class="form-group row">
               <label for="input" class="col-sm-4 col-form-label">Package Charges </label>
               <div class="col-sm-8">
                 {{$package_charge}}
               </div>
            </div>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer">
           <a href="<?php echo route('admin.voucher.package_voucher_sendmail',[$order_id]) ?>" title="Package Voucher" class="btn btn-success cab_view_fancy btn_admin" id="sendMail"> Send Mail </a>
