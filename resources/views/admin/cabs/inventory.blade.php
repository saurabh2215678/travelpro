  ...
  <div class="col-md-12">
    <div class="bgcolor">
      ...
      <div class="page_btns">
        ...
        <a <?php if($city->id == $city_id){ echo 'class="active"';} ?> href="{{route('admin.cabs.inventory',[$city->id])}}" title="Cabs Inventory">{{$city->name}}</a>
        <?php } ?>
      </div>
    </div>
  </div>
                      ...
                      <div class="dateselect"><span class="btn_date"><i class="fa fa-calendar"></i><input type="text" name="bulkdaterange" value="" /></span>
                        ...
                        <table class="table table-borderless">
                          <tbody class="date_input_box">
                            <tr>
                              <?php foreach ($cabsCityPrices as $key => $cabsCityPrice) { ?>
                            ...
                            <tr>
                              ...
                              <td style="width: 100px;" class="date">
                                <input type="text" class="form-control bulk_cab_inventory" id="" placeholder="" style="width: 80px;margin: 0 auto;" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" name="cab_inventory[{{$cabsCityPrice->cab_id}}]">
                              </td>
                              <?php } ?>
                            </tr>
                          </tbody>
...
<script type="text/javascript">
  ...
  function validateBulkInventory() {
    ...
    $.ajax({
      url: "{{route($routeName.'.cabs.bulk_inventory',['city_id'=>$city_id]) }}",
      type: "POST",
  ...
  function validateInventory() {
    ...
    var boxText = $('#'+formID).find('.btnSubmit').html();
    $.ajax({
      url: "{{route($routeName.'.cabs.saveAllInventory',['city_id'=>$city_id]) }}",
      type: "POST",
      data: new FormData($('#'+formID)[0]),
      processData: false,
      contentType: false,
  ...
  function loadInventory(date) {
    ...
    $.ajax({
      url: "{{route($routeName.'.cabs.nextInventory') }}",
      type: "POST",
      data: formData,
      dataType:"JSON",
      headers:{'X-CSRF-TOKEN': _token},
