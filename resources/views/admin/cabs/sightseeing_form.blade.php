@component('admin.layouts.main')
    ...
    if(empty($back_url)){
        $back_url = $routeName.'/cabs/sightseeing';
    }
    //pr($blog->toArray());
    $routeName = CustomHelper::getAdminRouteName();
    ...
    $duration_type = (isset($cab_route->duration_type))?$cab_route->duration_type:'';
    $duration_value = (isset($cab_route->duration_value))?$cab_route->duration_value:'';
    ...
    <?php if($id){ ?>
    <div class="page_btns">
        <a class="active" href="{{route($routeName.'.cabs.sightseeing_edit', ['id'=>$id])}}" title="Edit Cab Route"><i class="fas fa-edit"></i>Cab Sightseeing</a>
                ...
                <div class="row">
                    <div class="col-md-12">
                       <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                        <label class="required" >Sightseeing Name:</label><br/>
                    ...
                    <div class="col-md-4">
                        <div class="form-group{{ $errors->has('duration_type') ? ' has-error' : '' }}">
                            <label class="" >Duration Type:</label><br/>
                            <select class="form-control" id="duration_type" name="duration_type">
                                <option value="days" {{($duration_type=='days')?'selected':''}} >Days</option>
                                <option value="hours" {{($duration_type=='hours')?'selected':''}} >Hours</option>
                            </select>
                            @include('snippets.errors_first', ['param' => 'duration_type'])                          
                        </div>
                    </div>

                    <div class="col-md-4">
                        <div class="form-group{{ $errors->has('duration_value') ? ' has-error' : '' }}">
                            <label class="" >Duration Value</label><br/>
                            <input type="text" id="duration_value" class="form-control" name="duration_value" value="{{ old('duration_value', $duration_value) }}" onkeyup="if(/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" />
                            @include('snippets.errors_first', ['param' => 'duration_value'])                          
                        </div>
                    </div>                    

                    <div class="col-md-4">
                        <div class="form-group{{ $errors->has('duration') ? ' has-error' : '' }}">
                            <label class="" >Duration Text: (e.g. 2-3 hrs, 01N-02D, Same Day etc.)</label><br/>
                    ...
                    <div class="col-md-4">
                       <div class="form-group{{ $errors->has('distance') ? ' has-error' : '' }}">
                        <label class="required" >Distance in km:</label><br/>
                    ...
                    <div class="form-group{{ $errors->has('places') ? ' has-error' : '' }}">
                        <label class="" >Places covered in sightseeing:</label><br/>
