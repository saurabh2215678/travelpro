  ...
  <div class="col-md-12">
    <div class="bgcolor">
      ...
      <div class="page_btns">
        ...
        <a <?php if($city_id == $city->id){ echo 'class="active"';} ?> href="{{route('admin.cabs.price',[$city->id])}}" title="Cab Price">{{$city->name}}</a>
        <?php } ?>
      </div>
    </div>
  </div>
...
<div id="">
  ...
  <div class="dateRow">
    <form method="POST" action="" accept-charset="UTF-8" role="form" id="cab_price" onSubmit="return validatePrice()" >
    </form>
</section>
<?php } ?>
@slot('bottomBlock')
<script type="text/javascript">
  function validatePrice() {
    var formID = 'cab_price';
    var _token = '{{ csrf_token() }}';
    var boxText = $('#'+formID).find('.btnSubmit').html();
    $.ajax({
      url: "{{route($routeName.'.cabs.saveAllPrice',['city_id'=>$city_id]) }}",
      type: "POST",
  ...
  function loadPrice(date='') {
    ...
    $.ajax({
      url: "{{route($routeName.'.cabs.allPrice') }}",
      type: "POST",
      data: formData,
      dataType:"JSON",
      headers:{'X-CSRF-TOKEN': _token},
