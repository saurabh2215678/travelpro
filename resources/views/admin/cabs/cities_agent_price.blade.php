...
?>
<div class="page_btns">
  <a href="{{route($routeName.'.cabs.cities_view', ['id'=>$city_id])}}" title="Edit City"><i class="fas fa-view"></i>Cab City</a>
  <a  class="active" href="{{route($routeName.'.cabs.cities_agent_price', ['id'=>$city_id])}}" title="Agent price"><i class="fas fa-edit"></i>Agent price</a>
</div>
  ...
  <div class="bgcolor">
    ...
    <div class="table-responsive" id="discount_form" style="display: none;margin-bottom: 10px;">
      ...
      <div class="col-md-6" id="update_section">
        ...
        <a href="{{route($routeName.'.cabs.cities_agent_price',['id'=>$city_id])}}" class="btn_admin2">Cancel</a>
      </div>  
    </div>
    ...
    <table class="table table-striped table-bordered table-hover">
      ...
      if(!empty($discount_module_to_group) && count($discount_module_to_group) > 0 || $discount_category_id == '-1'){?>
        ...
        foreach ($cabsCityPrices as $key => $cabsCityPrice) {
          $price = $cabsCityPrice->price;
          ?>
          <tr>
            <td>{{$cabsCityPrice->cabData->name??''}}</td>
            <td>{{$price}}</td>
            <td>
              <?php
              $publish_price = $price;
...
<script type="text/javaScript">
  ...
  function save_discoount_category(){
    ...
    $.ajax({
      url: "{{ route('admin.cabs.cities_add_agent_price') }}",
      type: "POST",
