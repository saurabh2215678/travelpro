...
.bgcolor { padding-top: 40px; padding-bottom: 40px; }
    ...
    $name = (isset($email->name))?$email->name:'';
    $url = (isset($email->url))?$email->url:'';
    $title = (isset($email->title))?$email->title:'';
    $subject = (isset($email->subject))?$email->subject:'';
    $json_request = (isset($email->json_request))?$email->json_request:'';
    $status = (isset($email->status))?$email->status:1;
    $template_id = (isset($email->template_id))?$email->template_id:'';
    $is_has_attachment = (isset($email->is_has_attachment))?$email->is_has_attachment:0;
    ?>
    <div class="row1">
        <div class="col-sm-12">
                ...
                <div class="payment-gateway-buttons" tab-btns>
                    ...
                    $i =1;
                        foreach ($PaymentGateways as $key => $PaymentGateway) { 
                                    ...
                                    if($PaymentGateway->id == 1){
                                      
                                       $cash = $PaymentGateway;
                                    }
                                    if($PaymentGateway->id == 2){
                                      
                                       $bank = $PaymentGateway;
                                    }
                                    if($PaymentGateway->id == 3){
                                       $razorpay = $PaymentGateway;
                                    }
                                    if($PaymentGateway->id == 4){
                                       $paypal = $PaymentGateway;
                                    }
                                    if($PaymentGateway->id == 5){
                                       $payumoney = $PaymentGateway;
                                    }
                                    if($PaymentGateway->id == 6){
                                     $stripe = $PaymentGateway;
                                    }
                                    if($PaymentGateway->id == 7){
                                        $tpsl = $PaymentGateway;
                                   }

                                    if($i == 1 && empty($active_id)){

                                       $active_id = $PaymentGateway->id;
                                    }
                        ?>
                        
                    <button class="btn btn-sm btn-success pull-right <?php if($active_id == $PaymentGateway->id ){ echo 'active'; } $i++;?>" data-tab="{{$PaymentGateway->id}}">{{$PaymentGateway->name}}</button>
                    <?php  } ?>
  ...
  <?php if($cash){ ?>
                    ...
                    <div class="tab_box <?php if($active_id ==1){ echo 'active'; }?>" data-tab="1">
                       @include('admin.payment_gateways.include_tab.cash')
                       
                   </div> <?php } ?>
                       <?php if($bank){ ?>
                        <!-- tab item -->
                        <div class="tab_box <?php if($active_id == 2){ echo 'active'; }?>" data-tab="2">
                         @include('admin.payment_gateways.include_tab.bank')
                     ...
                     <?php if($razorpay){ ?>
                        <!-- tab item -->
                        <div class="tab_box <?php if($active_id == 3){ echo 'active'; }?>" data-tab="3">
                           @include('admin.payment_gateways.include_tab.razorpay')
                       </div>
                       <!-- tab item end -->
                   <?php } ?>
                       <?php if($paypal){ ?>
                        <!-- tab item -->
                        <div class="tab_box <?php if($active_id == 4){ echo 'active'; }?>" data-tab="4">
                            @include('admin.payment_gateways.include_tab.paypal')
                        </div>
                        <!-- tab item end -->
                    <?php } ?>
                    <?php if($payumoney){ ?>
                        ...
                        <!-- tab item -->
                        <div class="tab_box <?php if($active_id == 5){ echo 'active'; }?>" data-tab="5">
                             @include('admin.payment_gateways.include_tab.payU')
                         </div>
                         <!-- tab item -->
                     <?php } ?>
                     <?php if($stripe){ ?>
                        <!-- tab item -->
                        <div class="tab_box <?php if($active_id == 6){ echo 'active'; }?>" data-tab="6">
                         @include('admin.payment_gateways.include_tab.stripe')
                     </div>
                 <?php } ?>
                 <?php if($tpsl){ ?>
                    <!-- tab item -->
                    <div class="tab_box <?php if($active_id == 7){ echo 'active'; }?>" data-tab="7">
                     @include('admin.payment_gateways.include_tab.tpsl')
                 </div>
                <?php } ?>
